...
contract GnosisHelpers is Script {
    ...
    function getTimelockAddress() internal view returns (address) {
        return vm.parseJsonAddress(vm.readFile("config/production.json"), ".addresses.BeHYPETimelock");
    }
    ...
    function _getTimelockScheduleTransaction(address to, bytes memory data, bool isLasts) internal view returns (string memory) {
        ...
        string memory timelockAddressHex = iToHex(abi.encodePacked(getTimelockAddress()));
    ...
    function _getTimelockExecuteTransaction(address to, bytes memory data, bool isLasts) internal view returns (string memory) {
